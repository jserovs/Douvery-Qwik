...
export function getDataViewedProducts() {
  ...
  if (viewed != null) {
    ...
    console.warn('No se encontraron datos en localStorage para la clave:', OPTIONS_KEY_VIEWED_PRODUCTS);
  }
}
export async function addToViewedProducts({dui}: {dui: string}) {
    ...
    if (sessionDExists) {
      ...
      try {
        const response = await fetch(`${urlServerNode}/api/viewed/productsDui/json`, {
          method: 'POST',
      ...
      const productExists = viewed.some((product: {dui: string}) => product.dui === dui);
      ...
      if (!productExists) {
        ...
        console.log('El producto ya existe en la lista de productos vistos:', {dui});
      }
    }
  }
  ...
  export async function transferViewedProductsToAPI() {
    // Verifica si la cookie SESSION_D existe
    const sessionDExists = document.cookie.split(';').some((item) => item.trim().startsWith('SESSION_D='));
...
export async function getDataViewedProduct  () {
  ...
  throw new Error(errorResponse.msg);
    ...
    return 'No se encontraron datos en localStorage para la clave: ' + OPTIONS_KEY_VIEWED_PRODUCTS;
  }
}

export function removeAllViewedProducts() {
  localStorage.removeItem(OPTIONS_KEY_VIEWED_PRODUCTS);
  console.log('Todos los elementos han sido eliminados de la lista de productos vistos.');
}
